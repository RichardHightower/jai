...
public class FileUploadAsyncTest {
     ...
     */
    @Test
    void uploadFileRequestAsync() throws Exception {
        client = OpenAIClient.builder().setApiKey("pk-123456789").setHttpClient(httpClientMock).build();

        final CompletableFuture<ClientResponse<UploadFileRequest, FileData>> responseFuture = client.uploadFileAsync(uploadFileRequest);

        ClientResponse<UploadFileRequest, FileData> response = responseFuture.get();
