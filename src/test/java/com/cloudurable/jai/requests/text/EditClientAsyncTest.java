...
public class EditClientAsyncTest {
     ...
     */
    @Test
    void editAsync() throws Exception {
        httpClientMock = new HttpClientMock();
        client = OpenAIClient.builder().setApiKey("pk-123456789").setHttpClient(httpClientMock).build();

        // Mock the response
        final HttpClientMock.RequestResponse requestResponse = httpClientMock.setResponsePostAsync("/edits", requestBody, responseBody);

        final ClientResponse<EditRequest, EditResponse> response = client.editAsync(editRequest).get();
